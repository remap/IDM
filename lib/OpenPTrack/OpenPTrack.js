...
OpenPTrackJS.normalize = function(data_orig, swapX){
    ...
    // swap coordinates for Chiparaki
    if(swapX){
        // normalize
        data.x = 1-(data.x + Math.abs(minX)) / (maxX + Math.abs(minX));
    } else {
        // normalize
...
removeLast = function(array) {
    return (array.splice(lastIndex(array), 1)); 
}
...
OpenPTrackJS.pushToActive = function(data_orig){
    ...
    data = this.normalize(data_orig, true);
